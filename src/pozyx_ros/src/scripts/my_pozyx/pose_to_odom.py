...
vicon_sub = rospy.Subscriber(tag0_topic_ID, PoseStamped, vicon_cb, queue_size=100)
odom_pub = rospy.Publisher(odometry_topic_ID, Odometry, queue_size=100)
...
while not rospy.is_shutdown():
    ...
    if counter > 0:
        ...
        odom.child_frame_id = tag0_frame_ID
        ...
        br = tf.TransformBroadcaster()
       	br.sendTransform((x,y,0),[pose.pose.orientation.x, pose.pose.orientation.y,pose.pose.orientation.z,pose.pose.orientation.w],rospy.Time.now(), "base_link","odom")
